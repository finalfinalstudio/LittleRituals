const el = {
  ...
  minimal: document.getElementById('minimalBtn'),
...
function restoreSettings() {
  try {
    ...
    if (settings.minimal) {
      ...
      el.minimal.textContent = 'on';
    }
    if (settings.background) {
      el.background.value = settings.background;
      el.backgroundValue.textContent = settings.background;
...
el.background.addEventListener('input', () => {
  ...
  el.backgroundValue.textContent = el.background.value;
  saveSettings();
});
...
el.minimal.addEventListener('click', () => {
  document.body.classList.toggle('minimal');
  el.minimal.textContent = document.body.classList.contains('minimal') ? 'on' : 'off';
  saveSettings();
});

el.sound.addEventListener('change', saveSettings);
el.closeDone.addEventListener('click', () => { el.overlay.hidden = true; });
