...
function setFrame(p){const i=Math.min(FRAME_COUNT,Math.floor(p*(FRAME_COUNT-1))+1);if(i===lastFrame)return;lastFrame=i;els.frame.style.opacity=.55;setTimeout(()=>{els.frame.src=`assets/growing-flower/${String(i).padStart(3,'0')}.webp`;els.frame.onload=()=>els.frame.style.opacity=1},90)}
